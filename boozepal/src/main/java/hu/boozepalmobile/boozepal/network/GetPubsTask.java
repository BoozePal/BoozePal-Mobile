...
public class GetPubsTask extends AsyncTask<String, Void, List<Pub>> {
    ...
    @Override
    protected List<Pub> doInBackground(String... params) {
        return getPubs();
    }

    @Override
    protected void onPostExecute(List<Pub> pubTypes) {
        super.onPostExecute(pubTypes);
        delegate.onTaskFinished(pubTypes);
    }

    private List<Pub> getPubs() {
        URL url = null;
        InputStream is = null;
        ...
        try {
            ...
            Gson gson = new Gson();
            List<Pub> pubs = gson.fromJson(sb.toString(), listType);
            result = pubs;
